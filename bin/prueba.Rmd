...
raw %>%
  # select(fungicide, strain, experiment = rep, dose, plate=replicate, colony_diameter) %>%
  # mutate(colony_diameter = ifelse(colony_diameter < 0, 0, colony_diameter)) %>%
  mutate(plate = ifelse(plate == 11, 1, plate)) %>%
  rio::export(file= "data/mycelial_growth.csv")
# str(dat)
...
dat %>% 
  ggplot() + 
  aes(x=concentration_ppm, y=colony_diameter, col=strain) +
  geom_point() + 
