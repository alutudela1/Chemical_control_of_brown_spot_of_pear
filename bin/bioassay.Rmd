...
```{r}
sev_prev<-rio::import("data/preventive_treatments.csv", sep=";", header=TRUE, dec=",")
sev_prev<-read.table("preventive_treatments.csv", sep=";", header=TRUE, dec=",")
...
```{r warning=FALSE}
sev_prev %>% ggplot() + aes(x=treatment, y = spots_per_fruit) + geom_boxplot() +  geom_boxplot(alpha=.5, width = .2) + geom_jitter(col=2) + geom_point(alpha=.7) + labs(x="Treatment", y="Severity (spots per fruit)")
...
```{r}
curativo_dat<-read.table("curative treatments.csv", sep=";", header=TRUE, dec=",")
